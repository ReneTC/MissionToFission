...
func _draw() -> void:
	...
	const offset := Vector2(-10.0, 4.5)
	draw_circle(marker_min_pos, 15.0, Color.GRAY)
	draw_string(ThemeDB.fallback_font, marker_min_pos + offset, str(int(accepted_range.x)), HORIZONTAL_ALIGNMENT_CENTER, -1, 12, Color.BLACK)
